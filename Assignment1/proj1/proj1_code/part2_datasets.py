...
def make_dataset(path: str) -> Tuple[List[str], List[str]]:
    ...
    files.sort()
    images_a = [path + '/' + x for i, x in enumerate(files) if i % 2 == 0]
    images_b = [path + '/' + x for i, x in enumerate(files) if i % 2 == 1]
    ### END OF STUDENT CODE ####
    ############################
...
def get_cutoff_frequencies(path: str) -> List[int]:
    ...
    with open(path, 'r') as cf_file:
        for line in cf_file:
            cutoff_frequencies.extend([int(i) for i in line.split()])
    ### END OF STUDENT CODE ####
    ############################
