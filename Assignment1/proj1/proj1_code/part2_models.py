...
class HybridImageModel(nn.Module):
    def __init__(self):
        ...
        """
        super().__init__()#notice modifying
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        # print(kernel_np)
        k = kernel_np.shape[0]
        kernel_np = np.reshape(kernel_np, (1, k, k))
        kernel_np = np.tile(kernel_np, (self.n_channels, 1, 1, 1))
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        Args:
            ...
            kernel: low pass filter to be applied to the image (c, 1, k, k) # one convolution core 
        Returns:
        ...
        ############################
        ### TODO: YOUR CODE HERE ###
        filtered_image = F.conv2d(input=x, weight=kernel, stride=(1, 1), padding=kernel.shape[2]//2, groups=self.n_channels)
